<template>
    <div class="tags">
        <div class="tag_div">
            <el-tag @close="deltag(index)" v-for="(item,index) in tagList" :key="index" :class="{'active': isActive(item.path)}" size="medium" closable>
...
export default {
    ...
    methods:{
        ...
        addTags(route){
            ...
            !isExist && this.tagList.push({path:route.fullPath,title:route.meta.title})
        ...
        deltag(index){
            ...
            //获取删除后应该调到哪个的tag
            const tag = this.tagList[index] ? this.tagList[index] : this.tagList[index-1]
            //判断tag是否删除完
            if(tag){
                //判断删除的tag是否 是当前页面
