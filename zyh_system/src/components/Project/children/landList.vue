<template>
  <div class="landList">
    <div class="screening">
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        <el-form-item label="项目类别：">
          <el-input v-model="formInline.user" placeholder="地产" disabled></el-input>
        </el-form-item>
      ...
      <el-dialog title="添加土地" :visible.sync="open" width="1300px" center @close="down('projectForm')">
    ...
    <land-list-edit ref="editDialog" @changeFuc="changeFuc"/>
  </div>
</template>
<script>
...
import landListEdit from "@/components/Project/dialog/landListEdit";
...
export default {
  ...
  methods: {
    ...
    changMenu() {
      this.$refs.form.validateField("involvedDomain");
    },
    //按钮操作
    operation(type, data) {
      let obj;
    ...
    submitForm(projectForm) {
      ...
      this.$refs[this.projectComponent].submitForm();
        this.$refs[projectForm].validate(valid => {
          if (valid) {
            if(this.dataObj.isConfer == 0 && this.dataObj.startAmt != ""){
            this.previeFuc();
            }
            if (this.dataObj.isConfer == 1) {
              this.previeFuc();
            }
            else{
              this.$message({
    ...
    subFuc() {
      switch (this.dataObj.projectTypeCode) {
        case 2000:
           publishProjectB(this.freeDataObj)
              .then(res => {
                if (res && res.errorCode === 0) {
                  this.$message({
                    showClose: true,
