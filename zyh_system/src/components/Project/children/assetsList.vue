<template>
  <div class="assetsList">
    <div class="screening">
      <el-form :inline="true" :model="formInline" class="demo-form-inline" id="one">
        <el-form-item label="项目类别：">
          <el-input v-model="formInline.user" placeholder="资产" disabled></el-input>
        </el-form-item>
      ...
      <el-dialog title="添加资产" :visible.sync="open" width="1300px" center>
            ...
            >
              ...
              <div class="form_div">
                <el-form-item label="土地面积：" prop="landAcreage">
                  <span class="spanInput">
                    <el-input v-model="dataObj.landAcreage" type="text" placeholder="土地面积  (单位：㎡)" maxlength="10"></el-input>
    ...
    <assets-list-edit ref="editDialog" @changeFuc="changeFuc"/>
  </div>
</template>
...
import assetsListEdit from '@/components/Project/dialog/assetsListEdit'
...
// import {upload,publishProjectA,publishProjectB,publishProjectB,domainV2Init} from '@/api'
// import {findCode,findCodeArr,sortArr} from '@/files/js/findCode'
  ...
  data() {
    return {
      ...
      dataObj: {
        ...
        nature:"", //性质
        transferForm:"", //过户形式
        changeApartment:"",   //是否改公寓
        makeLoans:"",  //是否做贷款
        ownershipCertificate:'',//权属证明
        remakeKey:"", //关键环节
        taxSituation:"", //税收情况
        dtlInf:"", //项目详情信息
        ownedStatus:"", //共有情况
        pledgeStatus:"", //抵押、租赁等其他项权利情况
  ...
  methods: {
    ...
    tijiao(){
      switch (this.dataObj.projectTypeCode) {
        case 1000:
          publishProjectA(this.freeDataObj)
            .then(res => {
              if (res && res.errorCode === 0) {
                this.$message({
                  showClose: true,
