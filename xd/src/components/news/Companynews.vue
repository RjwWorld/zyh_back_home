<template>
  <div class="companynews">
    <div class="screening">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
          ...
          >添&nbsp;加</el-button>
          <el-dialog title="新闻添加" :visible.sync="open" width="1200px" center @close="down('newslistform')">
    ...
    <table-list
      ...
      @changeFuc="changeFuc"
    />
    <viewnews ref="lookDialog" @subFuc="subFuc"/>
    <newseditor ref="editDialog" @changeFuc="changeFuc"/>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: allTableHeader.companyheader,
      tableData: {},
      dataObj: {
        title: "", //标题
        author: "", //作者
  ...
  methods: {
      ...
      switch (type) {
        case "up":
          ...
          this.$confirm("确定发布该项目?", "提示", {
            ...
            .then(() => {
              companymodifyStatus(obj).then(res => {
                if (res && res.errorCode == 0) {
                  this.changeFuc();
                  this.$message({
                    showClose: true,
        ...
        case "del":
          ...
          break;
        case "look":
          companydtl({ id: data.id }).then(res => {
            if (res && res.errorCode === 0) {
    ...
    changeFuc(type, value) {
      ...
      companyNews(this.tableObj).then(res => {
        if (res && res.errorCode == 0) {
