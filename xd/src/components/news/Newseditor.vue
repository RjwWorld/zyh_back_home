<template>
  <div class="EditDialog">
    <el-dialog title="编辑" :visible.sync="showDialog" width="1200px" center @close="down('Dialoglistform')">
      <div class="editing">
        <el-form
          :model="dataObj"
          ref="Dialoglistform"
        ...
        <div class="submitBtn">
          <el-button type="primary" :loading="loadingBtn" @click="editFuc('Dialoglistform')">修&nbsp;改</el-button>
        </div>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    //关闭弹窗的回调
    down(Dialoglistform){
      this.$refs[Dialoglistform].resetFields();
    },
    //富文本编辑框
    ...
    editFuc(Dialoglistform) {
      this.$refs[Dialoglistform].validate(valid => {
        if (valid) {
